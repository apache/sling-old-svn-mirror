...
public class FormatFilterExtension implements RuntimeExtension {
    ...
    public Object call(final RenderContext renderContext, Object... arguments) {
        ...
        String source = SlingRuntimeObjectModel.toString(arguments[0]);
        Object[] params = decodeParams(arguments[1]);
        return replace(source, params);
    }

    private Object[] decodeParams(Object paramObj) {
        if (SlingRuntimeObjectModel.isCollection(paramObj)) {
            return SlingRuntimeObjectModel.toCollection(paramObj).toArray();
        }
    ...
    private String param(Object[] params, int index) {
        if (index >= 0 && index < params.length) {
            return SlingRuntimeObjectModel.toString(params[index]);
        }
        return "";
    }
}
