...
public abstract class RenderUnit implements Record<RenderUnit> {
    ...
    protected void callUnit(PrintWriter out, RenderContext renderContext, Object templateObj, Object argsObj) {
        if (!(templateObj instanceof RenderUnit)) {
            ...
            if (RuntimeObjectModel.isPrimitive(templateObj)) {
                throw new SightlyJavaCompilerException("data-sly-call: primitive \"" + templateObj.toString() + "\" does not represent a Sightly " +
                    "template.");
            } else if (templateObj instanceof String) {
                throw new SightlyJavaCompilerException("data-sly-call: String '" + templateObj.toString() + "' does not represent a Sightly template.");
            }
            throw new SightlyJavaCompilerException("data-sly-call: " + templateObj.getClass().getName() + " does not represent a Sightly template.");
        }
        RenderUnit unit = (RenderUnit) templateObj;
        Map<String, Object> argumentsMap = RuntimeObjectModel.toMap(argsObj);
