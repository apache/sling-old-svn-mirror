...
public class URLMapperFilter implements ComponentFilter, URLMapper {
    ...
    */
    ...
    public void doFilter(ComponentRequest request, ComponentResponse response,
        ...
        ContentManager cm = contentManagerFactory.getContentManager(requestData.getSession());
        requestData.setContentManager(cm);

        // 1.6 URL Mapping / Content Resolution --> URL Mapper
        Content content = getMappedURL(requestData);
        if (content == null) {
    ...
    public Content getMappedURL(RequestData requestData) {
        ...
        try {
            ...
            return transformURL(requestData, requestURI);
    ...
    public String handleToURL(String handle) {
        return handleToURL(null, handle, null);
    }
    ...
    private Content transformURL(RequestData requestData, String url) {
        ...
        for (int i = 0; i < mappings.length; i++) {
            ...
            Content content = findItem(pm, href);
            if (content == null) {
    ...
    private class MapperComponentResponse extends ComponentResponseWrapper {
        ...
        public String encodeURL(String path) {
            return super.encodeURL(handleToURL(contextPath, path, null));
        }
...
//        }
    ...
    private static class URLMapperConfig {
        ...
        void addMappings(Collection mappings,
            ...
            if (!isValid()) {
                log.debug("addMappings: mapperConfig={} invalid", toString());
                return;
            }

            if (handleExpander == null || from.length() == 0) {

                log.debug("addMappings: Add unexpanded mapping from {} to"
                    + " {}, dir={}", new Object[] { from, to, dir });
                mappings.add(new Mapping(from, to, dir));
                ...
                // } else {
                fromList = new String[] { from };
                addSlash = false;
                // }
                for (int i = 0; i < fromList.length; i++) {
                    // add slash lost during expansion
                    if (addSlash) fromList[i] += "/";
                    log.debug(
                        "addMappings: Add mapping from {} to {}, dir={}",
                        new Object[] { fromList[i], to, dir });
                    mappings.add(new Mapping(fromList[i], to, dir));
                }
        ...
        public String toString() {
            if (stringValue != null) {
                ...
                buf.append(", dir=").append(dir).append("]");
