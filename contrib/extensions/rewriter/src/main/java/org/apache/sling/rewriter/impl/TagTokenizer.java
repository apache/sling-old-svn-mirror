    ...
    private void attributeEnded() {
        if (attName.size() > 0) {
            if (hasAttributeValue) {
                attributes.addAttribute(attName.toString().toLowerCase(), attValue.toString(),
                        quoteChar);
            } else {
                attributes.addAttribute(attName.toString().toLowerCase(), quoteChar);
...
class AttributeListImpl implements AttributeList {
    ...
    /**
     * @see org.apache.sling.rewriter.impl.AttributeList#getQuoteChar(java.lang.String)
     */
