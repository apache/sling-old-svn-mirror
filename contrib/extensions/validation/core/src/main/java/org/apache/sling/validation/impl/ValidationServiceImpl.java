...
public class ValidationServiceImpl implements ValidationService, EventHandler {
     ...
     *            the type of resource for which to scan the JCR repository for validation models
                    ...
                    try {
                        ...
                        if (r != null) {
                            ...
                            if (!resourceProperties.isEmpty()) {
                                ...
                                vm = new JCRValidationModel(jcrPath, resourceProperties, validatedResourceType,
    ...
    /**
     * Checks if the {@code validationModel} does not override an existing stored model given the fact that the
     * overlaying is done based on the order in which the search paths are in the {@code searchPaths} array: the lower
     * the index, the higher the priority.
     *
     * @param validationModel
     *            the model to be checked
     * @param currentSearchPath
     *            the current search path
     * @param searchPaths
     *            the available search paths
     * @param validationModels
     *            the existing validation models
    ...
    private boolean canAcceptModel(JCRValidationModel validationModel, String currentSearchPath, String[] searchPaths,
        ...
        // perform null check to optimise performance in callee - no need to previously create the Trie if we're not
        // going to accept the model
            ...
            for (String searchPath : searchPaths) {
                if (!currentSearchPath.equals(searchPath)) {
                    for (String applicablePath : validationModel.getApplicablePaths()) {
                        ...
                        if (existingVM != null) {
                            String existingModelRelativeModelPath = existingVM.getJcrPath()
                                    .replaceFirst(searchPath, "");
    ...
    private void validatePropertyValue(ValidationResultImpl result, String property, String relativePath,
            ValueMap valueMap, List<ParameterizedValidator> validators) {
        for (ParameterizedValidator validator : validators) {
            // convert the type always to an array
            Class<?> type = validator.getType();
            if (!type.isArray()) {
                try {
                    // https://docs.oracle.com/javase/6/docs/api/java/lang/Class.html#getName%28%29 has some hints on
                    // class names
            ...
            if (typedValue == null || typedValue[0] == null) {
                ...
                result.addFailureMessage(relativePath + property, "Property was expected to be of type '" + validator.getType()
                        + "' but cannot be converted to that type.");
                return;
            }

            // see https://issues.apache.org/jira/browse/SLING-662 for a description on how multivalue properties are
            // treated with ValueMap
    ...
    private void validateValue(ValidationResultImpl result, Object value, String property, String relativePath,
            ValueMap valueMap, ParameterizedValidator validator) {
        try {
            @SuppressWarnings("unchecked")
            String validatorMessage = ((Validator) validator.getValidator()).validate(value, valueMap,
                    validator.getParameters());
            if (validatorMessage != null) {
                if (validatorMessage.isEmpty()) {
                    validatorMessage = "Property does not contain a valid value for the "
                            + validator.getClass().getName() + " validator";
                }
            ...
            throw new SlingValidationException("Could not call validator " + validator.getClass().getName()
                    + " for resourceProperty " + relativePath + property, e);
        }
    }
