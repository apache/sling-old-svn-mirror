...
public class IncludeTagHandler extends TagSupport {
    ...
    public int doEndTag() throws JspException {
        ...
        if (content != null) {
            ...
            Component component = TagUtil.getComponent(pageContext);
            ComponentRequestDispatcher crd = component.getComponentContext().getRequestDispatcher(content);
            
            // include the rendered content
            try {
                ComponentRequest request = TagUtil.getRequest(pageContext);
                ComponentResponse response = new JspComponentResponseWrapper(pageContext);        
                crd.include(request, response);
            } catch (IOException ioe) {
                throw new JspException("Error including " + content, ioe);
            } catch (ComponentException ce) {
                throw new JspException("Error including " + content, TagUtil.getRootCause(ce));
            }
        } else {
