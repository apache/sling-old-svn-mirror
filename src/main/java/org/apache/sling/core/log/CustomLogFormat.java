...
class CustomLogFormat {
    ...
    String format(ComponentRequest request, LoggerResponse response) {
        if (logParameters != null) {
            ...
            for (int i=0; i < logParameters.length; i++) {
                logParameters[i].print(buf, request, response);
            }
            return buf.toString();
        }
    ...
    public String toString() {
        StringBuffer buf = new StringBuffer();
        for (int i=0; logParameters != null && i < logParameters.length; i++) {
            buf.append(logParameters[i]);
        }
        return buf.toString();
    }
        ...
        for (int c=sr.first(); c != CharacterIterator.DONE; c=sr.next()) {
            if (c == '%') {
                ...
                if (c1 != '%') {
                    ...
                    Parameter param = parseFormatString(sr, c1);
                    if (param != null) {
    ...
    private Parameter parseFormatString(CharacterIterator sr, int c) {
        ...
        while (c != CharacterIterator.DONE) {
            if (c == '!') {
                ...
                statCodes = parseStatusCodes(sr, c);
            } else if (c == '>' || c == '>') {
    ...
    abstract static class BaseParameter implements Parameter {
        ...
        public final void print(StringBuffer dest, ComponentRequest request, LoggerResponse response) {
            if (printOk(response.getStatus())) {
                String value = isRequest ? getValue(request) : getValue(response);
                dest.append((value == null) ? "-" : value);
            }
        }
        
        protected boolean printOk(int status) {
            if (statusLimits == null) {
                return true;
            }
            
            for (int i=0; i < statusLimits.length; i++) {
                if (status == statusLimits[i]) {
                    return required;
                }
            }
        ...
        public String toString() {
            ...
            if (statusLimits != null) {
                ...
                for (int i=0; i < statusLimits.length; i++) {
                    ...
                    result.append(statusLimits[i]);
                }
            }

            if (parParam != null) {
                result.append('{').append(parParam).append('}');
            }
    ...
    static class TimeParameter extends BaseParameter {
        ...
        protected String getValue(LoggerResponse response) {
            long time = requestStart ? response.getRequestStart() : response.getRequestEnd();
    ...
    static class CookieParameter extends BaseParameter {
        ...
        protected String getValue(ComponentRequest request) {
            Cookie cookie = RequestUtil.getCookie(request, cookieName);
    ...
    static class HeaderParameter extends BaseParameter {
        ...
        protected String getValue(LoggerResponse response) {
            return escape(response.getHeaders(headerName));
        }
    }
}
