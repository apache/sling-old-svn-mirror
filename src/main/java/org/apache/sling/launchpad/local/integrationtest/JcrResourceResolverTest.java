...
public class JcrResourceResolverTest extends RepositoryTestBase {
    ...
    public class TestMapEntries extends MapEntries {

        public TestMapEntries(MapConfigurationProvider factory, BundleContext bundleContext, EventAdmin eventAdmin)
    ...
    protected void setUp() throws Exception {
        ...
        PrivateAccessor.setField(providerFactory, "repository", getRepository());
        ...
        resFac = new ResourceResolverFactoryImpl(activator);

        final EventAdmin mockVoidEA = mock(EventAdmin.class);
        final ServiceTracker voidTracker = mock(ServiceTracker.class);
        when(voidTracker.getService()).thenReturn(mockVoidEA);

        mapEntries = new TestMapEntries(resFac, mock(BundleContext.class), mockVoidEA);
...
*/
    ...
    public void testResolveResourceExternalRedirect() throws Exception {
        ...
        localhost443.setProperty(MapEntries.PROP_REDIRECT_EXTERNAL,
            "http://localhost");
        session.save();
