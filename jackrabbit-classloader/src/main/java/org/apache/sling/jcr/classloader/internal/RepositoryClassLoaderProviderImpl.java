...
public class RepositoryClassLoaderProviderImpl
    ...
    public ClassLoader getClassLoader(String owner) {
        String classLoaderOwner = getClassLoaderOwner(owner);
        RepositoryClassLoaderFacade loader =
            (RepositoryClassLoaderFacade) loaders.get(classLoaderOwner);
        if (loader == null) {
            loader = new RepositoryClassLoaderFacade(this, parent,
                classLoaderOwner, classPath);
            loaders.put(classLoaderOwner, loader);
        }
    ...
    protected void activate(ComponentContext componentContext) {
        ...
        classPath = (prop instanceof String[]) ? (String[]) prop : CLASS_PATH_DEFAULT;
        
        prop = properties.get(OWNER_PROP);
        classLoaderOwner = (prop instanceof String)? (String) prop : OWNER_DEFAULT;
        ...
        parent = new BundleProxyClassLoader(owner, null);
    }

    protected void deactivate(ComponentContext componentContext) {
        for (Iterator ci=loaders.values().iterator(); ci.hasNext(); ) {
