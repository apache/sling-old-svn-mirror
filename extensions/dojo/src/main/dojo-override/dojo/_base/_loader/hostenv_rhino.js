...
dojo._loadUri = function(uri, cb){
	try{
    	...
    	// START: Change for SLING-429
    	var script = sling.getScript().getScriptResource();
    	var resolver = script.getResourceResolver();
	    //normalize the uri (replace /./ with /)
    	uri = uri.replace(/\/\.\//g,"/");
    	var resource = resolver.resolve(uri);
    	var local = resource ? true : false;
    	// END: Change for SLING-429
...
function readText(path, encoding){
	...
	//normalize the uri (replace /./ with /)
	path = path.replace(/\/\.\//g,"/");
	var resource = resolver.resolve(path);
	var is = resource.adaptTo(java.io.InputStream);
