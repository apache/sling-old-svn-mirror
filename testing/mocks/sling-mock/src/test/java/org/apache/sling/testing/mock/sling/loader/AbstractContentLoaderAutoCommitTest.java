...
public abstract class AbstractContentLoaderAutoCommitTest {
    ...
    private void doTestBinaryFileAutocommitExplicitly(final boolean commit, final Runnable assertion) {
        ...
        loader.binaryFile(CLP_CONTENT, nextDestResource(), commit);
        assertion.run();

        loader.binaryFile(CLP_CONTENT, nextDestResource(), MIME_TYPE_JSON, commit);
        assertion.run();

        final InputStream data = new ByteArrayInputStream(MEM_CONTENT);
        loader.binaryFile(data, nextDestResource(), commit);
        assertion.run();

        final InputStream data2 = new ByteArrayInputStream(MEM_CONTENT);
        loader.binaryFile(data, nextDestResource(), MIME_TYPE_JSON, commit);
        assertion.run();

        final InputStream data3 = new ByteArrayInputStream(MEM_CONTENT);
        final Resource r = context.create().resource(nextDestResource());
        loader.binaryFile(data3, r, "child", commit);
        assertion.run();

        final InputStream data4 = new ByteArrayInputStream(MEM_CONTENT);
        final Resource r2 = context.create().resource(nextDestResource());
        loader.binaryFile(data4, r2, "child", MIME_TYPE_JSON, commit);
        assertion.run();
    }
    ...
    public void testBinaryFileAutocommitImplicitly() {
        ...
        final InputStream data2 = new ByteArrayInputStream(MEM_CONTENT);
        loader.binaryFile(data, nextDestResource(), MIME_TYPE_JSON);
        assertion.run();
    ...
    private void doTestBinaryResourceAutocommitExplicitly(final boolean commit, final Runnable assertion) {
        ...
        loader.binaryResource(CLP_CONTENT, nextDestResource(), commit);
        assertion.run();

        loader.binaryResource(CLP_CONTENT, nextDestResource(), MIME_TYPE_JSON, commit);
        assertion.run();

        final InputStream data = new ByteArrayInputStream(MEM_CONTENT);
        loader.binaryResource(data, nextDestResource(), commit);
        assertion.run();

        final InputStream data2 = new ByteArrayInputStream(MEM_CONTENT);
        loader.binaryResource(data, nextDestResource(), MIME_TYPE_JSON, commit);
        assertion.run();

        final InputStream data3 = new ByteArrayInputStream(MEM_CONTENT);
        final Resource r = context.create().resource(nextDestResource());
        loader.binaryResource(data3, r, "child", commit);
        assertion.run();

        final InputStream data4 = new ByteArrayInputStream(MEM_CONTENT);
        final Resource r2 = context.create().resource(nextDestResource());
        loader.binaryResource(data4, r2, "child", MIME_TYPE_JSON, commit);
        assertion.run();
    }
    ...
    public void testBinaryResourceAutocommitImplicitly() {
        ...
        final InputStream data2 = new ByteArrayInputStream(MEM_CONTENT);
        loader.binaryResource(data, nextDestResource(), MIME_TYPE_JSON);
        assertion.run();
